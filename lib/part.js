...
internals.parseReadme = (path, result, data, cb) => {
  const slug = `${result.result.unitDir}/${result.result.partDir}`;
  const tokens = Marked.lexer(data);

  if (!tokens.length) {
    result.log.push(['error', `README.md de ${slug} está vacío!`]);
  ...
  result.result.title = titleToken.text.replace(/^\d+\.\s/, '');
  ...
  Object.assign(result.result, internals.parseHeader(header, tokens.links));

  if (body) {
    result.result.body = Common.tokensToHTML(body, tokens.links);
    result.result.body = internals.parseYouTubeVideos(result.result.body);
    //result.result.body = internals.parseImages(cwd, result.result, result.result.body);
  }

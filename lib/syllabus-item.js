...
internals.tokensToPart = (currSection = 'title') => (memo, token) => {
  ...
  else if (token.type === 'heading' && token.depth === 2 && /objetivos/i.test(token.text)) {
    currSection = 'learningObjectives';
  }
  else if (currSection === 'body' && (!memo.duration || !memo.format) && token.type === 'list_start') {
    currSection = 'logistics';
  }
  else if (currSection === 'logistics' && token.type === 'list_end') {
  //else if (currSection !== 'body' && token.type === 'list_end') {
  //  if (memo[currSection] && memo[currSection].push) {
...
internals.parsePartFile = (cwd, part, cb) => {
  ...
  Async.series([
    ...
    if (results[1].learningObjectives && results[1].learningObjectives.length) {
      results[1].learningObjectives = Common.parseTokens(results[1].learningObjectives, []);
    }

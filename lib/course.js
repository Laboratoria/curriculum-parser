...
const parseSyllabus = (tokens, links) => tokens
  .reduce((memo, token) => {
    if (token.type === 'heading' && token.depth === 3) {
      ...
      memo[memo.length - 1].parts = token.cells.map(row => Object.assign({}, {
...
const parseTokens = (currSection = 'title') => (memo, token) => {
...
const parseCourseReadme = readme => promisify(fs.readFile)(readme, 'utf8')
  ...
  .then((tokens) => {
    ...
    const parsed = tokens.reduce(parseTokens(), defaults());
    ...
    return Object.keys(parsed).reduce((memo, key) => {
      if (key === 'title') {
        ...
        memo[key] = parseSyllabus(parsed[key], tokens.links);
      } else if (key !== 'tags') {
...
module.exports = (dir, opts = {}) => {
  ...
  return Promise.all([parseCourseReadme(readme), syllabus(dir)])
    .then(([course, syllabus]) => merge(course, syllabus))
    ...
    .then(json => validate('Topic', json).then(() => json));
};

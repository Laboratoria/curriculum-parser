...
import { computeCourseStats } from './stats.js';
...
const parseSyllabus = (tokens, links) => tokens
  ...
  .map(item => ({
    ...item,
    description: tokensToHTML(item.description, links),
  }));


const parseDescription = (tokens, links) => {
  const html = tokensToHTML(tokens, links);
...
const parseCourseReadme = readme => fs.readFile(readme, 'utf8')
  ...
  .then(async (tokens) => {
    ...
    const { cover, thumb } = await getImages(path.dirname(readme), tokens);
    ...
    return Object.keys(parsed).reduce((memo, key) => {
      ...
      if (key !== 'tags') {
        return { ...memo, [key]: tokensToHTML(parsed[key], tokens.links) };
      }
      return memo;
    }, { cover, thumb });
  })
...
export default (dir, opts = {}) => {
  ...
  const { slug } = parseDirname(readme);
  ...
  return Promise.all([parseCourseReadme(readme), syllabus(dir, opts)])
    ...
    .then(async result => applyGlobalOptions(
      result,
      opts,
      relativePath,

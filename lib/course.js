...
internals.parseSyllabusItemDuration = str => {
  ...
  1.5h (self-paced)
  */
  const matches = /^([0-9\.]+(h|min))\s+\((presencial|self-paced)\)$/.exec(str);

  if (!matches) {
    throw new Error('Syllabus item first paragraph must be duration string, ie: 15min (presencial)');
  }
...
internals.parseSyllabusItemTitle = str => {
  ...
  const matches = /^(((Lección|Lesson|Workshop) \d+):?\s+)?(\[.*\])(\(.*\)).*/.exec(str);
  ...
  let type = 'other';
  if (matches[3] === 'Lección' || matches[3] === 'Lesson') {
    type = 'lesson';
  }
  else if (matches[3] === 'Workshop') {
    type = 'workshop';
  }

  return {
    type,
    title: matches[4].slice(1, matches[4].length - 1),
    href: matches[5].slice(1, matches[5].length - 1)
  };
};

...
export const transformLearningObjectives = async (dir, opts, meta) => {
  ...
  let nonOptionalLearningObjectives = learningObjectives;
  ...
  if (!opts.lo || !existsSync(`${opts.lo}/data.yml`)) {
    return {
      ...
      optionalLearningObjectives: optionalLearningObjectives.length === 0
        ? undefined : optionalLearningObjectives,
    };
  }
  ...
  nonOptionalLearningObjectives = reduceAndSetChildren(nonOptionalLearningObjectives, known);

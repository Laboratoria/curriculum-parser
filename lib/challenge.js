...
const parseForm = async (children) => {
  const questionsHeadingIdx = children.findIndex(node => (
    ...
    && /(Preguntas)/i.test(node.children[0].value)
  ));
  ...
  const questions = questionsNodes.reduce(
    (memo, node) => {
      ...
      if (
        ...
        && node.children?.length
        && node.children[0]?.type === 'text'
        && /(Opciones)/i.test(node.children[0].value)
      ) {
        Object.assign(current, { options: [] });
        return memo;
      }

      if (current?.options && node.type === 'list' && node.ordered) {
      ...
      if (
        ...
        && node.children[0]?.value.startsWith('<solution')
        && node.children[2]?.type === 'html'
        && node.children[2]?.value === '</solution>'
      ) {
        const { value } = node.children[1];
        const valueAsArray = value.split(',').map(str => parseInt(str, 10) - 1);
        Object.assign(current, {
          solution: (
            !current.options
              ? value

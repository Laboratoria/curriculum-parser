...
internals.useloomLinkToEmbed = ($, $el, { v, ...qs }) => {
  const iframe = $('<iframe  width="640" height="360" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen>');
...
internals.parseLinks = (body) => {
  ...
  $('a').each((i, el) => {
    ...
    if (urlObj.hostname === 'www.useloom.com' && /share/.test(urlObj.pathname) ){
      return internals.useloomLinkToEmbed($, $el,  {
        ...qs,

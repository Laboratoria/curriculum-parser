...
internals.replaceLinkWithIframeAudio = ($, $el, iframe) => {
  const audioContainer = $('<div class="audio-container">');
  audioContainer.append(iframe);
  $el.replaceWith(audioContainer);
};
...
internals.soundcloudLinkToEmbed = ($, $el, { v, ...qs }) => {
  const iframe = $('<iframe  width="640" height="180" frameborder="0" allow="autoplay" scrolling="no">');
  const param = 'color=%23ffe521&auto_play=false&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true'
  iframe.attr('src', `https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/${v}&${param}`);

  internals.replaceLinkWithIframeAudio($, $el, iframe);
};

internals.slideLinkToEmbed = ($, $el, { v, ...qs }) => {
  const iframe = $('<iframe  width="640" height="360" frameborder="0">');
  iframe.attr('src', `https://docs.google.com/presentation/d/e/${v}`);
  iframe.attr('allowfullscreen', true);
  iframe.attr('mozallowfullscreen', true);
  iframe.attr('webkitallowfullscreen', true);
...
internals.parseLinks = (body) => {
  ...
  $('a').each((i, el) => {
    ...
    if (urlObj.hostname === 'docs.google.com' && /presentation/.test(urlObj.pathname)) {
      addEmbed('slide', 'docs.google', href, urlObj.pathname.slice(18));
      return internals.slideLinkToEmbed($, $el,  {
        ...qs,
        v: urlObj.pathname.slice(18),
      });
    }
